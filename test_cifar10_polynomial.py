...
# model = ResNet(Bottleneck, [3, 4, 6, 3], num_classes=10, custom_conv_layer_index=CUSTOM_CONV_LAYER_INDEX).cpu()
...
with torch.inference_mode():
    ...
    for image, target in metric_logger.log_every(data_loader_test, print_freq, header):
        ...
        mae = torch.abs(output_pure - output_polynomial).mean().item()
